...
class AdminHotelFeaturePricesSettingsController extends ModuleAdminController
{
    ...
    public function processSave()
    {
        ...
        if ($dateSelectionType == 2) {
            $isPlanTypeExists = $roomTypeFeaturePricing->checkRoomTypeFeaturePriceExistance($roomTypeId, $specificDate, date('Y-m-d', strtotime($specificDate) + 86400), 'specific_date');
        } else if (isset($isSpecialDaysExists) && $isSpecialDaysExists == 'on') {
            $isPlanTypeExists = $roomTypeFeaturePricing->checkRoomTypeFeaturePriceExistance($roomTypeId, $dateFrom, $dateTo, 'special_day', $jsonSpecialDays);
        } else {
            $isPlanTypeExists = $roomTypeFeaturePricing->checkRoomTypeFeaturePriceExistance($roomTypeId, $dateFrom, $dateTo, 'date_range');
        }
        if ($isPlanTypeExists) {
            $this->errors[] =  $this->l('A feature price plan already exists which some dates are common with this plan. Please select a different date range.');
        } else {
            if (!$roomTypeId) {
            ...
            if ($dateSelectionType == 1) {
                ...
                if (isset($isSpecialDaysExists) && $isSpecialDaysExists == 'on') {
                    ...
                    if (!isset($specialDays) || !count($specialDays)) {
            ...
            if (!count($this->errors)) {
                ...
                $this->errors[] = $this->l('Some error occurred while saving feature price plan.');
            }
        }
        if (isset($id) && $id) {
            $this->display = 'edit';
