...
class AdminOrdersControllerCore extends AdminController
{
    ...
    public function renderView()
    {
        ...
        if ($order_detail_data) {
            foreach ($order_detail_data as $key => $value) {
                ...
                $roomTotalPrice = $cart_detail_data_obj->getRoomTypeTotalPrice($value['id_product'], $value['date_from'], $value['date_to']);
                $tax_excl_price = $roomTotalPrice['total_price_tax_excl'];
                $tax_incl_price = $roomTotalPrice['total_price_tax_incl'];
    ...
    public function ajaxProcessEditProductOnOrder()
    {
        ...
        if ($bookedRooms) {
            foreach ($bookedRooms as $roomInfo) {
                if ($roomInfo['id_room'] == $id_room) {
                    $roomTotalPrice = $hotelCartBookingData->getRoomTypeTotalPrice($roomInfo['id_product'], $new_date_from, $new_date_to);
                } else {
                    $roomTotalPrice = $hotelCartBookingData->getRoomTypeTotalPrice($roomInfo['id_product'], $roomInfo['date_from'], $roomInfo['date_to']);
                }
    ...
    public function ajaxProcessDeleteProductLine()
    {
        ...
        $diff_products_tax_incl = $order_detail->total_price_tax_incl;
        $diff_products_tax_excl = $order_detail->total_price_tax_excl;
