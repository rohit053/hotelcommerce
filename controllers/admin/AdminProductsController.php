...
class AdminProductsControllerCore extends AdminController
{
    ...
    {
        foreach ($disableDates as $disable_key => $dates) {
            if (!$dates['dateTo'] && !$dates['dateFrom']) {
                unset($disableDates[$disable_key]);
            }
        }
        if (!count($disableDates) || !count($disableDates)) {
            $this->errors[] = Tools::displayError('Please enter disable dates for status temporary disable.');
        }
        if (!count($this->errors)) {
            foreach ($disableDates as $disable_key => $disableDates) {
                foreach ($disableDates as $key => $disDate) {
                    if ($key != $disable_key) {
                        if ((($disableDates['dateFrom'] < $disDate['dateFrom']) && ($disableDates['dateTo'] <= $disDate['dateFrom'])) || (($disableDates['dateFrom'] > $disDate['dateFrom']) && ($disableDates['dateFrom'] >= $disDate['dateTo']))) {
                        } else {
        ...
        if ($checkConfSubmit) {
            ...
            if (!count($this->errors)) {
                ...
                if ($room_num) {
                    ...
                    foreach ($room_status as $status_key => $status) {
                        if ($status == 3) {
                            $this->validateDisableDateRanges(Tools::jsonDecode($disable_dates[$status_key], true));
                        }
                    }
                    ...
                    if (!count($this->errors)) {
                        ...
                        foreach ($room_num as $key => $value) {
                            if ($value) {
                                ...
                                $obj_room_info->disabled_dates = $disable_dates[$key];
